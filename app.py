...
# ============================================================
st.set_page_config(
    page_title="Focus Oasis Foundation Accounting Portal",
    page_icon="FO",
    layout="wide",
...
# match the old portal's sheet structure, so reusing that same sheet
# would collide with it. Create a brand-new Google Sheet for this app,
...
LIGHT_GREY = "#D5DCE3"
FAINT_BLUE = "#F4F6F9"
HOVER_PRIMARY = "#E8EEF4"

# ============================================================
# CSS - FOCUS OASIS FOUNDATION THEME
# ============================================================
def inject_css():
    st.markdown(f"""
    <style>
...
#   "Journal Entries"  -> Date | Description | Debit Account | Credit Account | Amount | Source
#
...
#   "Journal Entries" -> Date | Description | Debit Account | Credit Account | Amount | Source
#
# Every account name below MUST match exactly what gets written into
# the "Debit Account" / "Credit Account" columns — the Trial Balance
# sums by exact string match, so a typo creates a phantom account with
# its own tiny balance instead of folding into the right one.
#
# Design choice, stated plainly: this ledger runs ALONGSIDE the
# existing Income Statement / Balance Sheet, not underneath them — the
# statements are still computed directly from the source sheets (Fee
# Payments, Expenses, etc.), exactly as before. The ledger is a second,
# genuinely double-entry view of the same transactions, for anyone who
# wants to see it as debits and credits. Revenue is recognised straight
# to Cash on receipt (no separate Debtors control account movement in
# the ledger), matching how the Income Statement already recognises it
# — Debtors on the Balance Sheet remains a computed memo figure rather
# than a ledger balance.
CHART_OF_ACCOUNTS = {
    # Account name: (Type, Normal Balance Side)
    "Cash and Cash Equivalents": ("Asset", "Debit"),
    "Debtors (Fees Receivable)": ("Asset", "Debit"),
    "Buildings": ("Asset", "Debit"),
    "Fixtures and Fittings": ("Asset", "Debit"),
    "Computer Software": ("Asset", "Debit"),
    "Motor Vehicles": ("Asset", "Debit"),
    "Computers": ("Asset", "Debit"),
    "Accumulated Depreciation": ("Asset", "Credit"),  # contra-asset

    "Creditors": ("Liability", "Credit"),
    "Short-term Loans": ("Liability", "Credit"),
    "Long-term Loans": ("Liability", "Credit"),
    "Tax Payable": ("Liability", "Credit"),
    "Deferred Income (Prepaid Fees)": ("Liability", "Credit"),

    "Ordinary Share Capital": ("Equity", "Credit"),
    "Share Premium": ("Equity", "Credit"),
    "Revaluation Reserve": ("Equity", "Credit"),
    "Retained Earnings": ("Equity", "Credit"),

    "Fee Income": ("Income", "Credit"),
    "Other Income": ("Income", "Credit"),

    "Staff Costs": ("Expense", "Debit"),
    "Utilities": ("Expense", "Debit"),
    "Teaching materials and supplies": ("Expense", "Debit"),
    "Repairs and maintenance": ("Expense", "Debit"),
    "Food and catering": ("Expense", "Debit"),
    "Transport": ("Expense", "Debit"),
    "Printing and stationery": ("Expense", "Debit"),
    "Events and extracurricular activities": ("Expense", "Debit"),
    "Sundry expenses": ("Expense", "Debit"),
    "Depreciation Expense": ("Expense", "Debit"),
    "Finance Costs": ("Expense", "Debit"),
...
def post_journal_entry(entry_date, description, debit_account, credit_account, amount, source):
    ...
    account, equal amount) to the Journal Entries sheet. Returns True/False."""
    if amount is None or amount == 0:
        return True  # nothing to post, not an error
    return write_data("Journal Entries", {
        "Date": str(entry_date),
...
def compute_trial_balance():
    """
    Returns a DataFrame with one row per account that has any activity:
    Account | Type | Debit | Credit — where exactly one of Debit/Credit
    is nonzero per row (the account's net balance, on whichever side it
    actually sits). Also returns (total_debit, total_credit), which
    should always be equal by construction of double-entry — if they
    ever aren't, some journal entry was posted with mismatched accounts.
    """
    df = load_journal()
    rows = []
    for account, (acc_type, _) in CHART_OF_ACCOUNTS.items():
        debit_side = df[df["Debit Account"] == account]["Amount"].sum() if not df.empty else 0.0
        credit_side = df[df["Credit Account"] == account]["Amount"].sum() if not df.empty else 0.0
        net = debit_side - credit_side
        if net == 0:
            continue
        rows.append({
            "Account": account, "Type": acc_type,
            "Debit": net if net > 0 else 0.0,
            "Credit": -net if net < 0 else 0.0,
        })
    df_tb = pd.DataFrame(rows, columns=["Account", "Type", "Debit", "Credit"])
    if not df_tb.empty:
        df_tb["_type_order"] = df_tb["Type"].apply(lambda t: ACCOUNT_TYPE_ORDER.index(t) if t in ACCOUNT_TYPE_ORDER else 99)
        df_tb = df_tb.sort_values(["_type_order", "Account"]).drop(columns=["_type_order"]).reset_index(drop=True)
    total_debit = df_tb["Debit"].sum() if not df_tb.empty else 0.0
    total_credit = df_tb["Credit"].sum() if not df_tb.empty else 0.0
    return df_tb, total_debit, total_credit


def compute_account_ledger(account):
    """Every journal entry touching one account, in date order, with a
    running balance on that account's own normal side."""
    df = load_journal()
    if df.empty:
        return pd.DataFrame(columns=["Date", "Description", "Debit", "Credit", "Balance", "Source"])
    mine = df[(df["Debit Account"] == account) | (df["Credit Account"] == account)].copy()
    if mine.empty:
        return pd.DataFrame(columns=["Date", "Description", "Debit", "Credit", "Balance", "Source"])
    mine["_date_parsed"] = pd.to_datetime(mine["Date"], errors="coerce")
    mine = mine.sort_values("_date_parsed", kind="stable")

    _, normal_side = CHART_OF_ACCOUNTS.get(account, ("Asset", "Debit"))
    balance = 0.0
    out_rows = []
    for _, r in mine.iterrows():
        debit_amt = r["Amount"] if r["Debit Account"] == account else 0.0
        credit_amt = r["Amount"] if r["Credit Account"] == account else 0.0
        movement = debit_amt - credit_amt
        balance += movement if normal_side == "Debit" else -movement
        out_rows.append({
            "Date": r["Date"], "Description": r["Description"],
            "Debit": debit_amt, "Credit": credit_amt,
            "Balance": balance, "Source": r.get("Source", ""),
        })
...
def login_page():
    ...
    <div class="top-banner">
        ...
        <h1>{SCHOOL_NAME}</h1>
        <div class="banner-subtitle">Accounting Portal</div>
    </div>
    """, unsafe_allow_html=True)
...
def admin_dashboard():
    ...
    if page == "Overview":
        ...
        admin_general_ledger_page()
    elif page == "Trial Balance":
        admin_trial_balance_page()
...
def admin_overview():
    ...
    ECD_STUDENTS_IMAGE_URL = "https://raw.githubusercontent.com/vimbainashesibanda4-byte/Focus-Oasis-Academy/main/Grow2-232-scaled.jpg"
    st.markdown(f"""
    ...
    with st.container(border=True):
        ...
        if st.button("Record Payment", use_container_width=True):
            if selected_label == "Select student...":
                ...
                if success:
                    credit_account = "Deferred Income (Prepaid Fees)" if academic_year > date.today().year else "Fee Income"
                    post_journal_entry(
                        payment_date, f"Fee payment - {student_name} ({academic_year})",
                        "Cash and Cash Equivalents", credit_account, amount, "Fee Payment",
                    )
    ...
    with st.container(border=True):
        ...
        if st.button("Record Expense", use_container_width=True):
            if amount <= 0:
                ...
                if success:
                    expense_account = EXPENSE_LINE_LABELS.get(category, "Sundry expenses")
                    post_journal_entry(
                        expense_date, description or category,
                        expense_account, "Cash and Cash Equivalents", amount, "Expense",
                    )
    ...
    with st.container(border=True):
        ...
        if st.button("Record Income", use_container_width=True):
            if amount <= 0:
                ...
                if success:
                    post_journal_entry(
                        income_date, description or "Other income",
                        "Cash and Cash Equivalents", "Other Income", amount, "Other Income",
                    )
...
def admin_fixed_assets_register():
    ...
    with st.container(border=True):
        ...
        with col2:
            ...
            paid_by = st.selectbox("Paid By", ["Cash", "On Credit (adds to Creditors)"], key="asset_paid_by")
        ...
        if st.button("Add Asset", use_container_width=True):
            if cost <= 0:
                ...
                if success:
                    credit_account = "Creditors" if paid_by.startswith("On Credit") else "Cash and Cash Equivalents"
                    post_journal_entry(
                        date_acquired, f"Purchased {description or category}",
                        category, credit_account, cost, "Fixed Asset",
                    )
                    if paid_by.startswith("On Credit"):
                        # Keep the Creditors register (which the Balance Sheet reads
                        # directly) consistent with what the ledger now shows.
                        write_data("Creditors", {
    ...
    with st.container(border=True):
        ...
        if st.button("Add Loan", use_container_width=True):
            if principal <= 0:
                ...
                if success:
                    post_journal_entry(
                        start_date, f"Loan from {lender}" if lender else f"{loan_type} loan",
                        "Cash and Cash Equivalents", f"{loan_type} Loans", principal, "Loan",
                    )
...
def admin_creditors_register():
    ...
    with st.container(border=True):
        ...
        st.caption("For an asset bought on credit (a building, a vehicle, etc.), use Fixed Assets Register instead — it has an 'On Credit' option that keeps everything in sync. This expense is recognised now, when incurred — when you eventually pay this creditor off, just delete its row from the Creditors tab directly rather than also recording a separate expense, or it'll be counted twice.")
        description = st.text_input("Description", key="cred_desc")
        expense_category = st.selectbox("Expense Category (which account gets debited)", list(EXPENSE_LINE_LABELS.keys()), key="cred_category")
        ...
        if st.button("Add Creditor", use_container_width=True):
            if amount <= 0:
                ...
                if success:
                    expense_account = EXPENSE_LINE_LABELS.get(expense_category, "Sundry expenses")
                    post_journal_entry(
                        cred_date, description or expense_category,
                        expense_account, "Creditors", amount, "Creditor",
                    )
                    # Mirror into Expenses too, so the Income Statement (which
                    # reads that sheet directly) reflects this cost immediately
                    # rather than only once the creditor is eventually paid.
                    write_data("Expenses", {
                        "Timestamp": str(datetime.now()),
                        "Date": str(cred_date),
                        "Description": f"{description} (on credit)" if description else f"{expense_category} (on credit)",
                        "Amount": amount,
...
def admin_equity_register():
    ...
    with st.container(border=True):
        ...
        revalued_asset = None
        if item == "Revaluation Reserve":
            revalued_asset = st.selectbox("Which asset is being revalued?", ASSET_CATEGORIES, key="equity_revalued_asset")
        ...
        if st.button("Add Equity Entry", use_container_width=True):
            ...
            if success:
                # Share capital/premium raise cash; a revaluation doesn't —
                # it debits the asset itself, not Cash.
                debit_account = revalued_asset if item == "Revaluation Reserve" else "Cash and Cash Equivalents"
                post_journal_entry(eq_date, f"{item} movement", debit_account, item, amount, "Equity")
...
def admin_general_ledger_page():
    ...
    with st.container(border=True):
        ...
        st.caption(
            "Staff costs, depreciation, finance costs and income tax aren't discrete cash "
            "transactions — they're computed once per financial year, the same way the Income "
            "Statement already computes them. Posting here adds them to the ledger so the Trial "
            "Balance reflects the full year, not just day-to-day cash transactions."
        )
        years = get_financial_years()
        adj_year = st.selectbox("Financial Year", years, key="ledger_adj_year")

        existing_journal = load_journal()
        already_posted = False
        if not existing_journal.empty:
            marker = f"Year-end adjustment for {adj_year}"
            already_posted = existing_journal["Description"].astype(str).str.contains(marker, regex=False).any()

        if already_posted:
            st.info(f"Year-end adjustments for {adj_year} are already posted. To redo them, delete those rows from the 'Journal Entries' tab directly first.")
        else:
            stmt = compute_income_statement(adj_year)
            st.markdown(f"""
            - Staff Costs: **${stmt['staff_costs']:,.2f}**
            - Depreciation: **${stmt['depreciation']:,.2f}**
            - Finance Costs: **${stmt['finance_costs']:,.2f}**
            - Income Tax: **${stmt['tax_expense']:,.2f}**
            """)
            if st.button(f"Post Year-End Adjustments for {adj_year}", use_container_width=True):
                year_end_date = date(adj_year, 12, 31)
                marker = f"Year-end adjustment for {adj_year}"
                ok = True
                ok &= post_journal_entry(year_end_date, marker, "Staff Costs", "Cash and Cash Equivalents", stmt["staff_costs"], "Year-End Adjustment")
                ok &= post_journal_entry(year_end_date, marker, "Depreciation Expense", "Accumulated Depreciation", stmt["depreciation"], "Year-End Adjustment")
                ok &= post_journal_entry(year_end_date, marker, "Finance Costs", "Cash and Cash Equivalents", stmt["finance_costs"], "Year-End Adjustment")
                ok &= post_journal_entry(year_end_date, marker, "Income Tax Expense", "Tax Payable", stmt["tax_expense"], "Year-End Adjustment")
                if ok:
                    st.success(f"Year-end adjustments posted for {adj_year}.")
                    st.rerun()
                else:
                    st.error("Some adjustments failed to post — check the 'Journal Entries' tab exists with the right headers.")

    df_journal = load_journal()

    with st.container(border=True):
        st.markdown("#### General Journal")
        st.caption("Every entry posted to date, in the order it was recorded.")
        if df_journal.empty:
    ...
    with st.container(border=True):
        ...
        if df_acc.empty:
            ...
            closing_balance = df_acc["Balance"].iloc[-1]
...
def admin_trial_balance_page():
    ...
    if df_tb.empty:
        st.info("No journal entries posted yet — record a transaction or post year-end adjustments first.")
        return
    ...
    check_color = GREEN if abs(total_debit - total_credit) < 0.01 else RED
    st.markdown(f"""
    <div style="border:2px solid {check_color}; border-radius:8px; padding:16px 20px; margin-top:10px;">
        <strong>Total Debits: ${total_debit:,.2f} &nbsp;&nbsp;|&nbsp;&nbsp; Total Credits: ${total_credit:,.2f}</strong><br>
        <span style="color:{check_color};">{"✓ Balanced" if abs(total_debit - total_credit) < 0.01 else "✗ NOT balanced — check for a journal entry with mismatched accounts."}</span>
    </div>
    """, unsafe_allow_html=True)
